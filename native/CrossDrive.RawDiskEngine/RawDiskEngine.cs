...
public sealed class RawDiskEngine : IRawDiskEngine
{
    ...
    public async Task<MountPlan> AnalyzeAsync(MountRequest request, CancellationToken cancellationToken = default)
    {
        ...
        using (rawDevice)
        {
            ...
            foreach (var candidate in candidates)
            {
                ...
                {
                    var detectedFamily = await DetectAppleHfsFamilyAsync(rawDevice, candidate.StartOffset, cancellationToken).ConfigureAwait(false);
                    var fsType = detectedFamily ?? "HFS+";
                    return new MountPlan(
                        slice.DevicePath,
                        fsType,
                        slice.Length,
                        Writable: false,
                        Notes: $"GPT {DescribeAppleHfsFamily(fsType)} partition detected at part {candidate.Index}; parser signature probe pending.",
                        PartitionOffsetBytes: candidate.StartOffset,
                        PartitionLengthBytes: candidate.Length
                    );
                }
            ...
            foreach (var mbr in mbrCandidates)
            {
                ...
                var detectedFamily = await DetectAppleHfsFamilyAsync(rawDevice, mbr.StartOffset, cancellationToken).ConfigureAwait(false);
                var fsType = detectedFamily ?? "HFS+";
                return new MountPlan(
                    slice.DevicePath,
                    fsType,
                    slice.Length,
                    Writable: false,
                    Notes: $"MBR {DescribeAppleHfsFamily(fsType)} partition (type 0xAF) at part {mbr.Index}; StartOffset={mbr.StartOffset}",
                    PartitionOffsetBytes: mbr.StartOffset,
                    PartitionLengthBytes: mbr.Length
                );
            }
            ...
            foreach (var apm in apmCandidates)
            {
                ...
                {
                    var detectedFamily = await DetectAppleHfsFamilyAsync(rawDevice, apm.StartOffset, cancellationToken).ConfigureAwait(false);
                    var fsType = detectedFamily ?? "HFS+";
                    return new MountPlan(
                        slice.DevicePath,
                        fsType,
                        slice.Length,
                        Writable: false,
                        Notes: $"APM {DescribeAppleHfsFamily(fsType)} partition detected at part {apm.Index}; Name={apm.Name}, StartOffset={apm.StartOffset}",
                        PartitionOffsetBytes: apm.StartOffset,
                        PartitionLengthBytes: apm.Length
                    );
                }
    ...
    private static async Task<string?> DetectAppleHfsFamilyAsync(IRawBlockDevice device, long partitionOffset, CancellationToken cancellationToken)
    {
        var signatureBuffer = new byte[2];
        var read = await RawReadUtil.ReadExactlyAtAsync(device, checked(partitionOffset + 1024), signatureBuffer, signatureBuffer.Length, cancellationToken).ConfigureAwait(false);
        if (read < signatureBuffer.Length)
        {
            return null;
        }

        return BinaryPrimitives.ReadUInt16BigEndian(signatureBuffer) switch
        {
            0x4244 => "HFS",
            0x482B => "HFS+",
            0x4858 => "HFSX",
    ...
    private static string DescribeAppleHfsFamily(string fsType) => fsType switch
    {
        "HFS" => "classic HFS",
        "HFSX" => "HFSX",
        _ => "HFS+"
