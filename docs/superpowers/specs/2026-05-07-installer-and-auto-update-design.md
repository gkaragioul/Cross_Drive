...
1. Ship CrossDrive as a normal Windows installer that shows the EULA and requires the user to accept it.
...
3. Releases live in a new public repo `georgekgr12/CrossDrive_Releases`. That repo holds nothing but a README and a LICENSE; every actual installer is a GitHub Release asset attached to a tag.
...
### `georgekgr12/CrossDrive` (existing source repo)
...
### `georgekgr12/CrossDrive_Releases` (new, public)
...
README.md     — Title, big Download badge linking to
                releases/latest/download/CrossDriveSetup.exe,
...
|---|---|---|
| `CrossDriveSetup.exe` | NSIS installer (assisted wizard, EULA, admin install) | **Yes** — both fresh install and in-app update download this. |
| `CrossDrive-<version>.exe` | Existing portable build | No — informational only, for users who don't want to install. |

Release notes body ends with a single line: `SHA256: <64-hex>` — the SHA256 of `CrossDriveSetup.exe`. The updater parses this and refuses to run a download whose hash doesn't match.
...
| `artifactName` | `"CrossDriveSetup.exe"` | Stable filename so `releases/latest/download/CrossDriveSetup.exe` works for the updater. |
...
|---|---|
| `GET /api/update/check?auto=0\|1` | ETag-conditional GET to `https://api.github.com/repos/georgekgr12/CrossDrive_Releases/releases/latest`. Compares the tag (`vX.Y.Z`) to `package.json` version. If newer (and, when `auto=1`, not in `dismissed_update.txt`), returns `{available:true, version, downloadUrl, sha256, releaseNotes}`. Otherwise `{available:false}`. ETag + body cached in `github_etag.txt` so a 304 response doesn't burn rate limit. |
| `POST /api/update/download` | Streams the asset to `%TEMP%\crossdrive_<guid>_CrossDriveSetup.exe`, computing SHA256 during write. On mismatch, deletes the file and returns 400. On success, returns the local path. |
...
### State files — `%LOCALAPPDATA%\CrossDrive\updates\`
...
Verbatim port of `MyLocalBackup.Core/Services/UpdateService.cs:399-415`. Emitted at runtime to `%TEMP%\crossdrive_relaunch_<guid>.ps1`. Steps:
...
4. `Start-Process` the new `CrossDrive.exe` from `%LOCALAPPDATA%\Programs\CrossDrive\CrossDrive.exe` (locked install path, so always known).
...
5. Compute SHA256 of `dist/CrossDriveSetup.exe`.
...
8. Run `gh release create v1.5.3 --repo georgekgr12/CrossDrive_Releases --notes-file <tempfile> dist/CrossDriveSetup.exe dist/CrossDrive-1.5.3.exe`.
9. Update `CrossDrive_Releases/README.md`'s "Recent Releases" line to `v1.5.3 — <date>` and push to that repo.
...
- `nsis.artifactName == "CrossDriveSetup.exe"`
- `routes/updateRoutes.js` exists
- `updateRoutes.js` references `github.com/georgekgr12/CrossDrive_Releases`
- `dist/CrossDriveSetup.exe` exists after `release:win:full`

`scripts/self-test.js` adds:
- `routes/updateRoutes.js` parses with `node --check` and exports a register function
- All four state-file paths in updateRoutes are absolute paths under `%LOCALAPPDATA%\CrossDrive\updates\`
...
1. **Releases repo bootstrap** — `gh repo create georgekgr12/CrossDrive_Releases --public`, copy LICENSE, write README from template, push initial commit. No source-repo changes.
